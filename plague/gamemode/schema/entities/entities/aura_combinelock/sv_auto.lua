...
function ENT:Think()
	if ( IsValid(self.entity) ) then
		if ( openAura.config:Get("combine_lock_overrides"):Get() ) then
...
function ENT:ActivateSmokeCharge(force)
	...
	if (self:GetDTFloat("smokeCharge") < curTime) then
		self:SetDTFloat("smokeCharge", curTime + 12);
		
		openAura:CreateTimer("smoke_charge_"..self:EntIndex(), 12, 1, function()
			if ( IsValid(self) ) then
				
				for k, v in ipairs(self.entities) do
					if (IsValid(v) and string.lower( v:GetClass() ) == "prop_door_rotating") then
						openAura.schema:BustDownDoor(nil, v, force);
...
function ENT:ToggleWithChecks(activator)
	...
	if (!self.nextUse or curTime >= self.nextUse) then
		if ( curTime > self:GetDTFloat("flash") ) then
			if ( curTime > self:GetDTFloat("smokeCharge") ) then
				self.nextUse = curTime + 3;
				
				if (!openAura.schema:PlayerIsCombine(activator) and activator:QueryCharacter("faction") != FACTION_ADMIN) then
